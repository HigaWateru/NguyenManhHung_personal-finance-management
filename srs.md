...
Thời gian cập nhật: 02:44 | 08/07/2026  
...
Tài liệu tham chiếu bao gồm đặc tả công nghệ của React, TypeScript, Vite, TailwindCSS, Redux Toolkit, Axios, React Router, Lucide React, Java 21, Spring Boot, Spring Security, Spring Data JPA, JWT, SQLite, Lombok, MapStruct, Gradle, JUnit5, Mockito, Spring Boot Test và MockMvc.
...
Hệ thống cung cấp quy trình đầy đủ từ đăng ký tài khoản đến quản lý giao dịch và phân tích tài chính. Người dùng có thể tạo và quản lý danh mục, ghi nhận thu nhập và chi tiêu theo từng thời điểm, tìm kiếm và phân trang dữ liệu lịch sử, xem dashboard tổng hợp và thống kê theo tháng, năm, danh mục. Hệ thống cung cấp quản lý hồ sơ người dùng, đổi mật khẩu, đăng xuất an toàn và cơ chế refresh token để duy trì phiên làm việc hợp lệ.
...
Frontend vận hành trên trình duyệt hiện đại hỗ trợ JavaScript ES2022 và CSS hiện đại. Backend chạy trên Java 21 với Spring Boot. Dữ liệu lưu trữ bằng SQLite cho giai đoạn hiện tại. Triển khai có thể thực hiện trên một máy chủ ứng dụng độc lập hoặc container hóa.

### 2.5 Ràng buộc thiết kế và triển khai
Ràng buộc công nghệ đã được xác định: React + TypeScript + Vite + TailwindCSS + Redux Toolkit ở phía client; Spring Boot + Spring Security + JPA + JWT + SQLite ở phía server. Thiết kế UI bắt buộc theo Dark Theme, Glassmorphism, Neon Blue và bảo đảm responsive.
...
| Backend | SQLite | Cơ sở dữ liệu quan hệ nhẹ |
...
Hệ thống áp dụng kiến trúc Client - Server tách lớp rõ ràng. Client React chịu trách nhiệm hiển thị, tương tác người dùng, quản lý state và giao tiếp API. Server Spring Boot xử lý nghiệp vụ, xác thực JWT, kiểm soát quyền truy cập và truy xuất dữ liệu qua JPA. SQLite là lớp lưu trữ lâu dài cho dữ liệu nghiệp vụ và token làm mới.
...
Kiến trúc module hóa cho phép bổ sung tính năng ngân sách, mục tiêu tiết kiệm, hoặc tích hợp nguồn dữ liệu ngoài mà không phá vỡ hợp đồng API hiện có. Tầng dữ liệu có thể chuyển từ SQLite sang hệ quản trị khác với thay đổi tối thiểu ở tầng repository và cấu hình.
...
Thiết kế cơ sở dữ liệu theo chuẩn quan hệ, chuẩn hóa ở mức phù hợp để giảm dư thừa và giữ truy vấn tổng hợp hiệu quả. Mọi bảng nghiệp vụ đều có trường thời gian tạo và cập nhật để hỗ trợ truy vết.
...
Hệ thống sử dụng JWT cho truy cập ngắn hạn và refresh token cho duy trì phiên. Token có thời hạn rõ ràng, có cơ chế thu hồi và kiểm tra trạng thái trước khi cấp mới.
...
Access token có thời lượng ngắn nhằm giảm rủi ro khi lộ token. Refresh token được lưu và quản lý vòng đời trong cơ sở dữ liệu để hỗ trợ thu hồi chủ động.
...
Các cấu hình tối thiểu bao gồm khóa bí mật JWT, thời hạn access token, thời hạn refresh token, cấu hình CORS, đường dẫn cơ sở dữ liệu SQLite, mức log và timezone hệ thống.
...
Trong các phiên bản tiếp theo có thể tích hợp dữ liệu giao dịch từ ngân hàng hoặc ví điện tử, đồng bộ đa thiết bị nâng cao và xuất báo cáo định dạng phong phú.

### 21.3 Mở rộng kiến trúc
Khi tải tăng cao, hệ thống có thể chuyển sang cơ sở dữ liệu quan hệ máy chủ chuyên dụng, thêm cache, bổ sung hàng đợi sự kiện và tách dịch vụ thống kê thành dịch vụ độc lập.
