...
- Sửa lỗi logic truy vấn sai thực thể `User` trên `CategoryRepository` và xử lý triệt để lỗi trùng khóa ngoại lệ `Duplicate entry ... for key 'categories.uk_categories_user_name_type'` bằng cách sử dụng `saveAndFlush` đồng bộ tức thời trong vòng lặp đồng bộ giao dịch Plaid.
