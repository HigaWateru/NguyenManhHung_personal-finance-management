...
export const validateLoginForm = (values: LoginFormValues) => {
  ...
  if (!values.password) errors.password = "Mật khẩu không được để trống.";
...
export const validateRegisterForm = (values: RegisterFormValues) => {
  ...
  if (!values.name.trim()) errors.name = "Họ tên không được để trống.";
  else if (values.name.trim().length < 2) errors.name = "Họ tên phải có ít nhất 2 ký tự.";
