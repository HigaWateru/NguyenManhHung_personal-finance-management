...
import { fetchExchangeRates } from "../../redux/slides/exchangeRateSlide"
...
export default function ExchangeRatePage() {
  ...
  useEffect(() => {
    ...
    if (fromRateObj && toRateObj) {
      // Conversion math: (amount * fromRateToVnd) / toRateToVnd
  ...
  const getRateFluctuationBadge = (change: number | null) => {
    if (change === null) return <span className="text-slate-400 font-normal">-</span>
  ...
  return (
    <div className="space-y-8 pb-12">
      {/* Top Banner / Hero */}
      <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-slate-900/50 p-6 md:p-8">
        ...
        <div className="relative flex flex-col justify-between gap-6 md:flex-row md:items-center">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-3 py-1 text-xs text-cyan-300">
              ...
              <span>Tiền tệ & Tỷ giá</span>
            </div>
            <h2 className="text-3xl font-bold tracking-tight text-white md:text-4xl">Tỷ Giá Thị Trường</h2>
            <p className="text-slate-400 max-w-3xl text-sm leading-6">
              Theo dõi tỷ giá ngoại tệ trực tuyến. Đơn vị tiền tệ hiển thị của toàn bộ hệ thống được đồng bộ tự động theo thiết lập trong trang hồ sơ tài khoản của bạn.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="glass-panel lg:col-span-1 rounded-3xl border border-white/10 bg-slate-900/40 p-6 space-y-6">
          <div className="flex items-center gap-3 border-b border-white/10 pb-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Quy đổi nhanh chóng giữa các mệnh giá</p>
            </div>
          </div>

          <div className="space-y-4">
            <div>
              <label htmlFor="convert-amount" className="block text-xs text-slate-400 mb-2">Số tiền cần quy đổi</label>
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-xl border border-white/10 bg-slate-950/80 px-4 py-3 text-white focus:border-cyan-500 focus:outline-none"
            ...
            <div className="grid grid-cols-[1fr_auto_1fr] gap-3 items-center">
              <div>
                <label htmlFor="from-currency" className="block text-xs text-slate-400 mb-2">Từ</label>
                <select
                  id="from-currency"
                  value={fromCurrency}
                  onChange={(e) => setFromCurrency(e.target.value as CurrencyCode)}
                  className="w-full rounded-xl border border-white/10 bg-slate-950/80 px-3 py-3 text-white focus:border-cyan-500 focus:outline-none"
                >
              ...
              <div className="flex justify-center pt-6">
                <button
                  ...
                  className="rounded-full border border-white/10 bg-white/5 p-3 text-slate-300 hover:bg-cyan-500 hover:text-white transition-colors duration-200"
              ...
              <div>
                <label htmlFor="to-currency" className="block text-xs text-slate-400 mb-2">Sang</label>
                <select
                  id="to-currency"
                  value={toCurrency}
          ...
          <div className="rounded-2xl bg-cyan-950/40 border border-cyan-800/30 p-5 space-y-2">
            <span className="text-xs uppercase tracking-wider text-cyan-400/80 font-medium">Kết quả quy đổi</span>
        ...
        <div className="glass-panel lg:col-span-2 rounded-3xl border border-white/10 bg-slate-900/40 p-6 space-y-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-white/10 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-white">Bảng Tỷ Giá Trực Tuyến</h3>
                <p className="text-xs text-slate-400">So với đơn vị chuẩn Việt Nam Đồng (VND)</p>
              </div>
            </div>
            ...
            <button
              ...
              disabled={loading}
              className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm font-semibold text-white hover:bg-white/10 disabled:opacity-50 transition-colors"
            >
                ...
                {rates.map((rate) => (
                  <tr key={rate.id} className="text-sm transition-colors hover:bg-white/5">
                    <td className="py-4 pl-4 font-medium text-white flex items-center gap-3">
                      <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-800 text-cyan-400 font-semibold border border-white/10">
                        {rate.symbol}
                      </div>
                      <div>
                        <div className="font-bold text-white">{rate.currencyCode}</div>
