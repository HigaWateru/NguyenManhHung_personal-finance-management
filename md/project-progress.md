...
### E. Đăng nhập mạng xã hội (OAuth2 Client) & Tối ưu Sandbox
- **Đăng nhập Google & GitHub**: Tích hợp Spring Security OAuth2 Client trên Backend để nhận thông tin xác thực từ các nhà cung cấp bên thứ ba (Google và GitHub), tự động đăng ký tài khoản mới khi đăng nhập lần đầu và đồng bộ hóa với hệ thống JWT stateless.
- **Trang chuyển hướng Frontend**: Tạo route `/oauth2/redirect` để nhận Access Token và Refresh Token từ Backend, lưu trữ an toàn trong localStorage và tự động đồng bộ hóa thông tin profile người dùng.
- **Giao diện Nút đăng nhập**: Bổ sung các nút đăng nhập bằng Google và GitHub đẹp mắt, nhất quán với phong cách thiết kế Cyber Finance của hệ thống.
- **Tối ưu cấu hình Plaid Sandbox**: Chuẩn hóa các tham số Plaid Sandbox, loại bỏ khoảng trắng dư thừa trong fallback values để tránh lỗi xác thực API, cấu hình Plaid API Version (`2020-09-14`) đáp ứng yêu cầu phiên bản Plaid mới nhất.
...
- **Đăng nhập mạng xã hội (OAuth2)**: `Completed` Đã liên kết và hỗ trợ đăng nhập nhanh qua Google hoặc GitHub.
