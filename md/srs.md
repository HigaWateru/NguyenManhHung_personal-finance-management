...
- Phân hệ Systemwide Multi-Language System (Đa ngôn ngữ i18n 100% hỗ trợ 🇻🇳 Tiếng Việt, 🇬🇧 English, 🇯🇵 日本語 kèm Icon cờ quốc gia trên Header, dịch toàn bộ Cards, Charts, Tables và Modals).
...
| i18n | Internationalization (Đa ngôn ngữ hệ thống) |
...
- **Systemwide Multi-Language System (i18n)**: Bộ chọn ngôn ngữ trên Header tích hợp **Cờ quốc gia** và tên ngôn ngữ hỗ trợ 3 ngôn ngữ (🇻🇳 Tiếng Việt, 🇬🇧 English, 🇯🇵 日本語), chuyển đổi đồng bộ 100% tất cả thẻ Card, Bảng biểu, Biểu đồ và Modal thoại, tự động lưu vào `localStorage`.
...
Giao diện bắt buộc thiết kế theo chuẩn Dark Theme, Glassmorphism, Neon Blue Cyber, bảo đảm responsive 100% trên cả Desktop và Mobile, hỗ trợ `cursor: pointer` chuẩn trên toàn bộ các tương tác nút bấm.
...
Bao gồm toàn bộ các module Xác thực, OAuth2, OTP Email, Plaid Sandbox (**First Platypus Bank**), Merchant Categorization Pipeline, Incomes, Expenses, Categories, Dashboard (Top Category Highlight), Budgets, Goals, Notifications, Exchange Rates, Multi-language System Systemwide (VI, EN, JA) và Clean DataSeeder Policy.

### 3.3 Phạm vi ngoài dự án
Không bao gồm kết nối ngân hàng tiền thật (Production Plaid/VN Banks) và giao dịch chuyển tiền trực tiếp giữa các ngân hàng.
...
- Được phép: 
  ...
  - Chuyển đổi ngôn ngữ hệ thống tức thì (Tiếng Việt 🇻🇳, English 🇬🇧, 日本語 🇯🇵).
...
- Tổng hợp chỉ số: Tổng số dư, Thu nhập tháng, Chi tiêu tháng, Tỷ lệ tiết kiệm (Dịch chuyển đổi động theo 3 ngôn ngữ).
...
### 8.5 Module Multi-Language System (Systemwide i18n)
- **Supported Languages**: Hỗ trợ 3 ngôn ngữ giao diện:
  - **Tiếng Việt (🇻🇳 `vi`)** - Ngôn ngữ mặc định
  - **English (🇬🇧 `en`)**
  - **日本語 (🇯🇵 `ja`)**
- **Header Language Selector**: Nút chọn ngôn ngữ tích hợp lá cờ quốc gia và tên ngôn ngữ (🇻🇳 Tiếng Việt, 🇬🇧 English, 🇯🇵 日本語), có dấu tích xanh cho ngôn ngữ hoạt động, cho phép đổi tức thì không reload trang.
- **100% Coverage**: Phủ kín 100% các thành phần UI (Header, Sidebar, 4 Card Lối tắt Module, 4 Thẻ Card Chỉ số, Biểu đồ Tuần/Phân bổ, Bảng Giao dịch gần đây, Widget Ngân sách/Mục tiêu/Hóa đơn định kỳ, Modal Tác vụ nhanh/Ngân sách/Mục tiêu/Ngân hàng, Bảng Thu nhập/Chi tiêu/Danh mục/Thống kê/Tỷ giá/Profile).
- **Persistence**: Tự động lưu và khôi phục ngôn ngữ lựa chọn của người dùng trong `localStorage` (`sf_language`).

---

## 9. Non Functional Requirements

1. **Hiệu năng (Performance)**: Thời gian phản hồi API đồng bộ và phân loại $< 1.5$s nhờ Redis Cache. Chuyển đổi ngôn ngữ và dữ liệu giao diện instant không giật lag.
...
4. **Giao diện (Usability/Design)**: Dark Theme, Glassmorphism, Neon Blue Cyber, responsive 100%, bổ sung `cursor: pointer` chuẩn.
