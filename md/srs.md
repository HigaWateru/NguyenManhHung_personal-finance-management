...
#### 8.8.7 Forgot Password (Quên mật khẩu)

| Thuộc tính | Nội dung |
|---|---|
| Mục đích | Khôi phục quyền truy cập tài khoản khi người dùng quên mật khẩu bằng mã OTP gửi qua Email. |
| Input | email, mã OTP 6 chữ số, newPassword, confirmPassword. |
| Output | Trạng thái gửi OTP, trạng thái xác thực OTP, xác nhận đổi mật khẩu thành công. |
| Business Rule | Sinh OTP ngẫu nhiên 6 chữ số, băm bằng BCrypt và lưu vào Redis TTL 5 phút. Giới hạn nhập sai tối đa 5 lần (sau 5 lần tự động hủy OTP). Đăng ký rate limit gửi OTP tối đa 1 lần/phút và 5 lần/giờ cho mỗi tài khoản. Sau khi reset thành công, cập nhật credentialsUpdatedAt và thu hồi toàn bộ RefreshToken đang hoạt động để yêu cầu đăng nhập lại ở mọi thiết bị. |
| Validation | Email đúng định dạng; OTP đúng định dạng 6 chữ số; mật khẩu mới đạt chính sách bảo mật; confirmPassword khớp newPassword. |
| Exception | OTP không tồn tại/hết hạn, OTP sai quá 5 lần, mật khẩu không đạt chính sách, rate limit bị kích hoạt. |
| API liên quan | POST /api/v1/auth/forgot-password, POST /api/v1/auth/verify-otp, POST /api/v1/auth/reset-password |
...
| bank_accounts | Lưu thông tin tài khoản ngân hàng kết nối Plaid | id | user_id -> users.id |
| transactions | Lưu giao dịch ngân hàng đã đồng bộ | id | user_id -> users.id, bank_account_id -> bank_accounts.id, category_id -> categories.id |
| budgets | Lưu hạn mức chi tiêu theo tháng | id | user_id -> users.id, category_id -> categories.id |
| goals | Lưu mục tiêu tiết kiệm | id | user_id -> users.id |
| notifications | Lưu cảnh báo và thông báo hệ thống | id | user_id -> users.id |
| merchant_mappings | Lưu cấu hình phân loại merchant | id | user_id -> users.id, category_id -> categories.id |
...
#### 12.2.6 POST /api/v1/auth/forgot-password

| Thành phần | Mô tả |
|---|---|
| URL | /api/v1/auth/forgot-password |
| Method | POST |
| Request | Body gồm email |
| Response | message thông báo gửi mã thành công (trả chung cho cả email tồn tại và không tồn tại) |
| Status Code | 200 thành công; 400 dữ liệu sai hoặc rate limit; 500 lỗi hệ thống |

#### 12.2.7 POST /api/v1/auth/verify-otp

| Thành phần | Mô tả |
|---|---|
| URL | /api/v1/auth/verify-otp |
| Method | POST |
| Request | Body gồm email, otp |
| Response | message xác nhận OTP hợp lệ |
| Status Code | 200 thành công; 400 mã sai, hết hạn hoặc bị hủy do sai quá 5 lần; 500 lỗi hệ thống |

#### 12.2.8 POST /api/v1/auth/reset-password

| Thành phần | Mô tả |
|---|---|
| URL | /api/v1/auth/reset-password |
| Method | POST |
| Request | Body gồm email, otp, password, confirmPassword |
| Response | message xác nhận đặt lại mật khẩu thành công |
| Status Code | 200 thành công; 400 mật khẩu không đạt yêu cầu hoặc không khớp, OTP chưa được xác thực; 500 lỗi hệ thống |
...
### 12.9 Plaid Integration APIs

#### 12.9.1 POST /api/v1/plaid/create-link-token
- **Mục đích**: Khởi tạo Link Token để mở Plaid Link ở Frontend.
- **Method**: POST
- **Request**: Header Authorization
- **Response**: Trả về `linkToken`
- **Status Code**: 200 thành công; 401 không xác thực; 500 lỗi Plaid API.

#### 12.9.2 POST /api/v1/plaid/exchange-public-token
- **Mục đích**: Nhận public_token từ Frontend, trao đổi lấy access_token và lưu kết nối ngân hàng.
- **Method**: POST
- **Request**: Body chứa `publicToken` và `institutionName`
- **Response**: Message xác nhận thành công
- **Status Code**: 200 thành công; 400 dữ liệu thiếu; 401 không xác thực.

#### 12.9.3 POST /api/v1/plaid/sync
- **Mục đích**: Đồng bộ giao dịch thủ công từ Plaid Sandbox và kích hoạt pipeline phân loại.
- **Method**: POST
- **Request**: Header Authorization
- **Response**: Trả về `syncedCount` (số lượng giao dịch đồng bộ mới)
- **Status Code**: 200 thành công; 401 không xác thực.

#### 12.9.4 GET /api/v1/plaid/accounts
- **Mục đích**: Lấy danh sách tất cả các tài khoản ngân hàng đã liên kết.
- **Method**: GET
- **Request**: Header Authorization
- **Response**: Mảng các tài khoản ngân hàng (tên, số dư, loại tài khoản, đồng bộ cuối)
- **Status Code**: 200 thành công; 401 không xác thực.

#### 12.9.5 GET /api/v1/plaid/status
- **Mục đích**: Kiểm tra người dùng hiện tại đã kết nối ngân hàng hay chưa.
- **Method**: GET
- **Request**: Header Authorization
- **Response**: Trả về boolean `connected`
- **Status Code**: 200 thành công; 401 không xác thực.

#### 12.9.6 POST /api/v1/plaid/disconnect
- **Mục đích**: Xóa kết nối ngân hàng và các giao dịch đã đồng bộ liên quan.
- **Method**: POST
- **Request**: Header Authorization
- **Response**: Message xác nhận thành công
- **Status Code**: 200 thành công; 401 không xác thực.

### 12.10 Budget Management APIs

#### 12.10.1 GET /api/v1/budgets
- **Mục đích**: Lấy danh sách ngân sách tháng hiện tại của người dùng.
- **Method**: GET
- **Response**: Danh sách ngân sách (hạn mức, số tiền đã chi tiêu, ngày bắt đầu/kết thúc)
- **Status Code**: 200 thành công; 401 không xác thực.

#### 12.10.2 POST /api/v1/budgets
- **Mục đích**: Thiết lập ngân sách mới cho một danh mục.
- **Method**: POST
- **Request**: Body chứa `categoryId`, `limitAmount`
- **Response**: Thông tin ngân sách vừa tạo
- **Status Code**: 200 thành công; 400 dữ liệu sai.

#### 12.10.3 PUT /api/v1/budgets/{id}
- **Mục đích**: Cập nhật hạn mức ngân sách chi tiêu.
- **Method**: PUT
- **Request**: Body chứa `limitAmount`
- **Response**: Thông tin ngân sách sau cập nhật
- **Status Code**: 200 thành công; 404 không tìm thấy.

#### 12.10.4 DELETE /api/v1/budgets/{id}
- **Mục đích**: Xóa ngân sách chi tiêu.
- **Method**: DELETE
- **Status Code**: 200 thành công; 404 không tìm thấy.

### 12.11 Saving Goals APIs

#### 12.11.1 GET /api/v1/goals
- **Mục đích**: Lấy danh sách mục tiêu tiết kiệm.
- **Method**: GET
- **Response**: Danh sách mục tiêu (tên, số tiền hiện tại, số tiền đích, ngày hoàn thành)
- **Status Code**: 200 thành công.

#### 12.11.2 POST /api/v1/goals
- **Mục đích**: Tạo mục tiêu tích lũy mới.
- **Method**: POST
- **Request**: Body chứa `name`, `targetAmount`, `targetDate`
- **Response**: Thông tin mục tiêu vừa tạo
- **Status Code**: 200 thành công.

#### 12.11.3 PUT /api/v1/goals/{id}/progress
- **Mục đích**: Cập nhật số tiền đã tích lũy được cho mục tiêu.
- **Method**: PUT
- **Request**: Body chứa `currentAmount`
- **Response**: Thông tin mục tiêu cập nhật
- **Status Code**: 200 thành công.

#### 12.11.4 DELETE /api/v1/goals/{id}
- **Mục đích**: Xóa mục tiêu tiết kiệm.
- **Method**: DELETE
- **Status Code**: 200 thành công.

### 12.12 Notifications APIs

#### 12.12.1 GET /api/v1/notifications
- **Mục đích**: Lấy thông báo hệ thống và cảnh báo chi tiêu của người dùng.
- **Method**: GET
- **Request**: Query `unreadOnly` tùy chọn (mặc định false)
- **Response**: Danh sách thông báo
- **Status Code**: 200 thành công.

#### 12.12.2 POST /api/v1/notifications/{id}/read
- **Mục đích**: Đánh dấu thông báo cụ thể là đã đọc.
- **Method**: POST
- **Status Code**: 200 thành open.

#### 12.12.3 POST /api/v1/notifications/mark-all-read
- **Mục đích**: Đánh dấu tất cả thông báo của người dùng là đã đọc.
- **Method**: POST
- **Status Code**: 200 thành công.
...
| otp | Bắt buộc, đúng định dạng 6 chữ số |
...
| BR-11 | Gửi OTP quên mật khẩu bị giới hạn tần suất tối đa 1 lần/phút và 5 lần/giờ cho mỗi tài khoản. |
| BR-12 | Nhập sai OTP quá 5 lần sẽ tự động hủy OTP trong hệ thống để tránh brute-force. |
| BR-13 | Đặt lại mật khẩu thành công sẽ tự động cập nhật credentialsUpdatedAt và thu hồi tất cả RefreshToken. |
